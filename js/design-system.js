...
:root {
  /* ── الألوان الأساسية ──────────────────────────────────────
     مستعارة الآن من primitives موحّدة في css/style.css (وليست
     قيماً خاماً مكررة هنا) — القيم نفسها لم تتغيّر، فقط أصبح لها
     مصدر واحد بدل تعريفين مستقلّين كانا بإمكانهما الانجراف عن
     بعضهما بمرور الوقت (وهذا بالضبط ما حدث قبل هذا الإصلاح). تبقى
     ثابتة عبر كل الثيمات عمداً — تُستخدَم كخلفيات صلبة مع نص أبيض
     في عدة أماكن (أزرار/badges)، فلو صارت متغيّرة مع الثيم لانكسر
     تباين النص الأبيض فوقها. أي استخدام كنص مباشر على خلفية تتغيّر
     لونها يجب أن يستخدم توكن دلالي متوافق مع الثيم بدل هذه مباشرة
     (--green-mid/--green-deep/--gold-dark/--sys-*) — كما فُعل بالفعل
     في كل نقاط الاستهلاك النصي المعروفة. */
  --zad-green-900: var(--primitive-green-900, #0e3b2e);
  --zad-green-700: var(--primitive-green-700, #1a5d47);
  --zad-green-500: var(--primitive-green-500, #2a7a5f);
  --zad-green-100: var(--primitive-green-100, #e8f4ef);

  --zad-gold-700:  var(--primitive-gold-700, #b87200);
  --zad-gold-500:  var(--primitive-gold-500, #c9a14a);
  --zad-gold-100:  var(--primitive-gold-100, #fff8e8);

  /* هذه الثلاثة استُهلكت تاريخياً كنص فقط (لا خلفية) في كل نقاط
     الاستخدام المعروفة، فتُستعار مباشرة من توكنز دلالية متوافقة مع
     الثيم أصلاً بدل primitive ثابت — أي استخدام جديد لها سيرث
     التوافق مع الوضع الداكن/OLED تلقائياً بدل الحاجة لإصلاح لاحق. */
  --zad-amber:     var(--sys-orange, #ff9500);
  --zad-coral:     var(--sys-red, #c0392b);
  --zad-purple:    var(--sys-purple, #4a3580);
